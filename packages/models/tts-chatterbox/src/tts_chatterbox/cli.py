"""Human-friendly CLI for Chatterbox-Turbo TTS synthesis.

Usage:
    uv run --project packages/models/tts-chatterbox python -m tts_chatterbox.cli \
        -t "Hello world" --reference-audio path/to/ref.wav
"""
...
from tts_chatterbox.synthesizer import LocalChatterboxSynthesizer
...
@click.option("--voice-id", default=None, help="Voice preset.")
...
def main(
    ...
    """Synthesize speech from text using Chatterbox-Turbo (voice cloning)."""
    logger.remove()
    ...
    logger.info("Reference audio: {}", reference_audio)
    ...
    synthesizer = LocalChatterboxSynthesizer()
