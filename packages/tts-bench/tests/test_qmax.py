    ...
    stub.add_response(
        "get_metric_statistics",
        {
            "Datapoints": [
                ...
                for i, (avg, mx) in enumerate(zip(concurrency_avg, concurrency_max, strict=True))
            ]
        },
    )
    stub.add_response(
...
class _RecordingRunner:
    ...
    def __call__(self, client: Any, **kwargs: Any) -> StepResult:
        ...
        return _result(
            ...
            hold_s=self._hold_s,
            outcome=(
                InvokeOutcome.SATURATED_503 if index in self._saturated_at else InvokeOutcome.OK
            ),
...
def _run_ladder(
    runner: _RecordingRunner, *, clock: Clock | None = None, **kwargs: Any
) -> LadderRun:
...
class TestRunLadder:
    ...
    def test_it_stops_after_two_consecutive_saturated_rungs(self) -> None:
        ...
        ladder = _run_ladder(_RecordingRunner(saturated_at=(1, 2)), concurrencies=(1, 5, 10, 20))
...
class TestJoinCloudwatch:
    ...
    def test_no_results_returns_the_rungs_untouched(self, cloudwatch: tuple[Any, Stubber]) -> None:
        client, stub = cloudwatch
        calls: list[dict[str, Any]] = []
        _record_metric_calls(client, calls)
        steps = _rungs(LADDER)
...
class TestTheLadderTablesDownstreamReads:
    ...
    def test_the_unit_ratio_is_measured_per_rung(self, cloudwatch: tuple[Any, Stubber]) -> None:
        ...
        report = _report(ladders=[_ladder(_rungs({**LADDER, **FAILING_RUNG}))], joined_steps=joined)
...
class TestMeasureRefusesBeforeItSendsLoad:
    def test_a_fleet_that_can_still_grow_stops_the_run(
        ...
        with pytest.raises(FixtureError, match="scale-out still active"):
            _measure(require_frozen=True, runtime_client=client, appscaling=aas, sagemaker=sm)
